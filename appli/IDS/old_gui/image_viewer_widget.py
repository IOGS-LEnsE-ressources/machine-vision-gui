# -*- coding: utf-8 -*-
"""*image_viewer_widget.py* file.

*image_viewer_widget* file that contains :class::ImageViewerWidget
...
class ImageViewerWidget(QWidget):
    """ImageViewerWidget class, children of QWidget.
    ...
    def __init__(self, zoom_params: bool = False) -> None:
        ...
        self.zoomed = False                 # If true, display a zoomed image if image is smaller than area
    ...
    def set_image_from_array(self, pixels: np.ndarray, zoomed: bool = False) -> None:
        """
    ...
    def resizeEvent(self, event):
        ...
        if not self.image_display.pixmap().isNull():
            h_area = self.image_area.height()
            w_area = self.image_area.width()
            if (h > h_area or w > w_area) or self.zoomed is True:
                self.image_display.setPixmap(self.image_display.pixmap().scaled(self.image_area.size(),
                                                                            Qt.AspectRatioMode.KeepAspectRatio,
                                                                            Qt.TransformationMode.SmoothTransformation))
            else:
                self.image_display.setPixmap(self.image_display.pixmap())
...
if __name__ == "__main__":
    ...
    class MyMainWindow(QMainWindow):
        ...
        def __init__(self) -> None:
            ...
            self.central_widget = ImageViewerWidget()
    ...
    array = np.random.randint(0, 255, size=(100, 200), dtype=np.uint8)
